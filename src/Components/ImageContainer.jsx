...
export default function ImageContainer({ allImageUrl, allImageId }) {
  ...
  const renderCount = 3; // Number of images to render at a time

  //initial images to display when none are clicked
  const initialImageId = allImageId.slice(startIndex, startIndex + renderCount);
  const initialImageUrl = allImageUrl.slice(startIndex, startIndex + renderCount);
...
//since we are displaying 3 images only one should be a clicked image
  ...
  const randomClickedImageObj = clickedImages.length > 0
  ...
  return (
    <div className="flex gap-2 justify-center h-[300px] w-full">
      {displayImages.map((img) => (
        <ImageButton key={img.id} src={img.url} onClick={() => handleImageClick(img.url, img.id)} />
      ))}
    </div>
  );
}
